...
namespace VNTourLandProject.Pages.Manage
{
    public class MyBookingModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            var response = await _bookingService.GetMyBookingAsync(CurrentUserId);

            if (response.IsSuccess && response.Result is List<MyBookingDTO> bookings)
            {
                Bookings = bookings;
            }
            else
            {
                Bookings = new();
                TempData["Message"] = "Unable to retrieve your bookings.";
            }
        ...
        // ✅ Lấy lịch sử tin nhắn với người dùng cụ thể
